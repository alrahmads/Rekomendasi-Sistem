...
st.title("Sistem Rekomendasi Lowongan Pekerjaan Data Science di Indonesia Menggunakan Content-Based Filtering pada Platform LinkedIn")
st.write("""
...
st.subheader("Silahkan filter lowongan pekerjaan yang sesuai dengan keinginan Anda:")
...
all_qualifications = df["Job Qualifications"].apply(lambda x: [item.strip() for item in x.split(",") if item.strip() != ""])
unique_qualifications = sorted(set([qual for sublist in all_qualifications for qual in sublist]))

# Tambahkan opsi kosong di awal agar user bisa memilih secara eksplisit
job_titles = df["Job Title"].unique()
job_types = df["Type"].unique()
work_hours = df["Work Hours"].unique()

selected_job = st.selectbox("Pilih Pekerjaan", options=job_titles, index=0)
selected_job_type = st.selectbox("Pilih Tipe Pekerjaan", options=job_types, index=0)
selected_work_hours = st.selectbox("Pilih Jam Kerja", options=work_hours, index=0)
selected_job_qualification = st.multiselect("Pilih Kualifikasi Pekerjaan", options=unique_qualifications)

# Gabungkan hanya jika ada isinya
combined_data = ' '.join(
    [selected_job] + selected_job_qualification + [selected_job_type, selected_work_hours]
)

st.write("Hasil gabungan:", combined_data)
